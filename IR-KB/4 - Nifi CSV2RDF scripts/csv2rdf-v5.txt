...
""")
        ...
        parser.records.each { record ->
            try {
                ...
                // Process Events and EventActions
                ...
                if (eventsJson) {
                    try {
                        ...
                        if (events && events.size() > 0) {
                            ...
                            eventUri = "<http://irkb.com/event/${incidentId}_${evtId}>"
                            events.eachWithIndex { evtAction, idx ->
                                def actionUri = "<http://irkb.com/event/${incidentId}_${evtId}_action_${idx + 1}>"
                ...
                if (eventUri) {
                    eventActionLinks.each { writer.write("    :hasEventAction ${it} ;\n") }
                }
                ...
                if (events && events.size() > 0) {
                    ...
                    events.eachWithIndex { evtAction, idx ->
                        def actionUri = "<http://irkb.com/event/${incidentId}_${evtId}_action_${idx + 1}>"
                        writer.write("    :hasEventAction ${actionUri} ;\n")
                    }
                    writer.write(".\n\n")

                    events.eachWithIndex { evtAction, idx ->
                        def actionUri = "<http://irkb.com/event/${incidentId}_${evtId}_action_${idx + 1}>"
                        writer.write("${actionUri} a :EventAction ;\n")
                        writeLiteral('hasEventActivity', evtAction['action'])
                        writeLiteral('hasEventDescription', evtAction['title'])  // <<< USE 'title' field as event description

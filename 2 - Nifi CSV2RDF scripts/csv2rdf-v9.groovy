...
""")
        ...
        def mapTaskTitleToCategory = { title ->
            ...
            if (lowerTitle =~ /(detection|identification|detect|identify)/) return 'Detection'
            if (lowerTitle =~ /analysis|analyses|analyse/) return 'Analysis'
            if (lowerTitle =~ /(containment|contain|isolation|isolate)/) return 'Containment'
            if (lowerTitle =~ /(eradication|eradicate|remediation)/) return 'Eradication'
            if (lowerTitle =~ /recovery|recover/) return 'Recovery'
